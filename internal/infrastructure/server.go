...
func NewServer(version, buildTag, runEnv string) (server *Server, err error) {
	...
	//minio, err := datasources.InitMinioClient()
	//if err != nil {
	//	return
	//}
	mqtt, err := datasources.MqttConnect(viper.GetString("mqtt.broker"), viper.GetString("mqtt.client_id"))
	if err != nil {
		return
	}
	// init app resources
	// server.Resources = NewResources(fastHTTPClient, mainDbConn, nil, server.RedisStorage, jwtResources, minio)
	server.Resources = NewResources(fastHTTPClient, nil, nil, nil, jwtResources, nil, mqtt)
...
func connectToRedis() (storage *redis.Storage, err error) {
